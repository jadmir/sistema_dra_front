<template>
  <q-page padding class="bg-grey-1">
            ...
            <div class="q-pa-xs col-12">
              <q-card bordered>
                <q-card-section>
                  ...
                  <div class="text-caption text-grey-7">{{ props.row.ubicacion || '—' }}</div>
    ...
    <!-- Form modal -->
    <destino-form v-model="showForm" :destino="selected" @saved="onSaved" @close="closeForm" />
  </q-page>
</template>
...
// --------- FUNCIONES ---------
function openForm(destino = null) {
  selected.value = destino ? { ...destino } : null
  showForm.value = true
}
...
function onSaved() {
  store.fetchDestinos(store.pagination.page)
  closeForm()
}

function onRequest({ pagination }) {
  store.pagination.page = pagination.page
  store.fetchDestinos(pagination.page)
}

function onSearch() {
  store.pagination.page = 1
  store.fetchDestinos(1)
}
...
async function toggleEstado(row) {
  ...
  const result = await Swal.fire({
    title: `¿Deseas ${accion} el destino "${row.nombre}"?`,
    icon: 'question',
  ...
  const ok = await store.deleteDestino(row.id, { estado: nuevo })
  if (ok) {
    Swal.fire({
      icon: 'success',
      title: `Destino ${nuevo ? 'activado' : 'desactivado'}`,
      timer: 1200,
