  ...
  <q-page padding>
    <div class="q-pa-md">
      ...
      <div class="row items-center q-mb-md">
        <div class="col">
          <div class="text-h5 text-weight-medium text-grey-8">
            <q-icon name="workspaces" color="green-7" size="sm" class="q-mr-sm" />
            Grupos
          </div>
          <div class="text-caption text-grey-6">Gestiona los grupos de cultivos</div>
        </div>
        <div class="col-auto gt-xs">
          <q-btn unelevated color="green-7" icon="add" label="Nuevo Grupo" @click="openForm()" />
        </div>
      </div>
      ...
      <q-card>
        ...
        >
          <!-- Desktop: subsector -->
          <template #body-cell-subsector="props">
            <q-td>
              {{ props.row.subsector?.descripcion || props.row.subsector?.codigo || '—' }}
            </q-td>
          </template>
              ...
              <q-card bordered class="cursor-pointer" @click="openDetail(props.row)">
                ...
                <q-list dense>
                  ...
                  <q-item>
                    ...
                    <q-item-section side>{{
                      props.row.subsector?.descripcion || '—'
                    }}</q-item-section>
                  </q-item>
                </q-list>
    ...
    <q-dialog v-model="detailOpen">
      <q-card style="max-width: 520px; width: 100%">
        <q-card-section class="row items-center q-gutter-sm bg-gradient-green text-white">
          <q-avatar color="white" text-color="green-7" icon="workspaces" />
        ...
        <q-list dense>
          ...
          <q-item>
            <q-item-section>Subsector</q-item-section>
            <q-item-section side>{{ detailItem?.subsector?.descripcion || '—' }}</q-item-section>
          </q-item>
        </q-list>
    ...
    <q-dialog v-model="showForm" persistent>
      <q-card style="min-width: 400px; max-width: 500px">
        <q-card-section class="bg-gradient-green text-white">
          <div class="text-h6">
            <q-icon name="workspaces" size="sm" class="q-mr-sm" />
        ...
        <q-card-section>
          <q-form ref="formRef" @submit.prevent="onSubmit">
            <q-select
              ...
              dense
              class="q-mb-md"
              :rules="[(v) => !!v || 'El subsector es requerido']"
...
const columns = [
  ...
  { name: 'subsector', label: 'Subsector', field: 'subsector', align: 'left', sortable: true },
...
const openForm = (item = null) => {
  ...
  if (item) {
    form.value = {
      subsector_id: item.subsector_id || item.subsector?.id,
      codigo: item.codigo,
...
const onSubmit = async () => {
  ...
  const success = isEdit.value
    ? await grupoStore.updateGrupo(selectedItem.value.id, form.value)
    : await grupoStore.createGrupo(form.value)
  ...
  if (success) {
    $q.notify({
      type: 'positive',
      message: isEdit.value ? 'Grupo actualizado' : 'Grupo creado',
    })
    closeForm()
  } else {
    $q.notify({ type: 'negative', message: grupoStore.error })
  }
}
...
const confirmDelete = async (id) => {
  ...
  if (res.isConfirmed) {
    const ok = await grupoStore.deleteGrupo(id)
    if (ok) await Swal.fire('Eliminado', 'El grupo fue eliminado', 'success')
    else await Swal.fire('Error', grupoStore.error || 'No se pudo eliminar', 'error')
  }
}
