<template>
  <q-page padding>
    <div class="q-pa-md">
      ...
      <div class="row items-center q-mb-md">
        <div class="col">
          <div class="text-h5 text-weight-medium text-grey-8">
            <q-icon name="category" color="green-7" size="sm" class="q-mr-sm" />
            Subsectores
          </div>
          <div class="text-caption text-grey-6">Gestiona los subsectores agrícolas</div>
        </div>
      ...
      <q-card>
        <q-table
          :rows="subsectorStore.subsectores"
          :columns="columns"
          row-key="id"
          :loading="subsectorStore.loading"
          :grid="isMobile"
          :dense="isMobile"
          flat
          bordered
    ...
    <q-dialog v-model="detailOpen">
      <q-card style="max-width: 520px; width: 100%">
        <q-card-section class="row items-center q-gutter-sm bg-gradient-green text-white">
          <q-avatar color="white" text-color="green-7" icon="category" />
      ...
      <q-card style="min-width: 400px; max-width: 500px">
        <q-card-section class="bg-gradient-green text-white">
          <div class="text-h6">
            ...
            {{ isEdit ? 'Editar Subsector' : 'Nuevo Subsector' }}
          </div>
        </q-card-section>
...
onMounted(() => {
  subsectorStore.fetchSubsectores()
})
...
const onSubmit = async () => {
  ...
  const success = isEdit.value
    ? await subsectorStore.updateSubsector(selectedItem.value.id, form.value)
    : await subsectorStore.createSubsector(form.value)
  ...
  if (success) {
    $q.notify({
      type: 'positive',
      message: isEdit.value ? 'Subsector actualizado' : 'Subsector creado',
    })
    closeForm()
  } else {
    $q.notify({ type: 'negative', message: subsectorStore.error })
  }
}
...
const confirmDelete = async (id) => {
  ...
  if (res.isConfirmed) {
    const ok = await subsectorStore.deleteSubsector(id)
    if (ok) await Swal.fire('Eliminado', 'El subsector fue eliminado', 'success')
    else await Swal.fire('Error', subsectorStore.error || 'No se pudo eliminar', 'error')
  }
}
